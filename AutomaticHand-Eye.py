...
with open('D:\CAMERA-IMU\github\DATA\cam.pkl', 'rb') as f:
    camdata_array = pickle.load(f)
with open('D:\CAMERA-IMU\github\DATA\imu.pkl', 'rb') as f:
    imudata_array = pickle.load(f)

cam_array = [arr for arr in camdata_array if not (arr == np.zeros((4, 4))).all()]  # 保留可以参与计算的帧间位姿
imu_array = [arr for arr in imudata_array if not (arr == np.zeros((4, 4))).all()]  # 保留可以参与计算的帧间位姿
# camdata_array = camdata_array[:68]
# imudata_array = imudata_array[:68]
print(camdata_array)
print(imudata_array)
Hcg = np.zeros((4, 4))
d = 0.1
Tsai_HandEye(Hcg, camdata_array, imudata_array)
print(len(imudata_array))
print(f'初始计算外参为：{Hcg}')
...
ransac_min_samples = 300  # 拟合模型所需的最小样本数
ransac_residual_threshold = 0.1  # 内点/外点确定的阈值
